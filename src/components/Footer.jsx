...
export default function Footer() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(docRef, (docSnap) => {
      ...
      console.error("Error loading storefront settings in footer:", error);
    });
    return unsubscribe;
  }, []);
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-12 mb-12">
        ...
        <div className="space-y-4">
          ...
          <p className="text-neutral-400 text-sm leading-relaxed">
            Premium everyday track pants designed for comfort and motion. 
            Upgrade your wardrobe with our latest drops.
          </p>
          <div className="flex space-x-4 pt-2">
            {storefrontSettings.socialInstagram && (
              <a href={storefrontSettings.socialInstagram} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-neutral-900 flex items-center justify-center text-neutral-400 hover:text-cyan-400 hover:bg-cyan-500/10 transition-colors">
            ...
            {storefrontSettings.socialFacebook && (
              <a href={storefrontSettings.socialFacebook} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-neutral-900 flex items-center justify-center text-neutral-400 hover:text-cyan-400 hover:bg-cyan-500/10 transition-colors">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
                  <path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/>
                </svg>
              </a>
            )}
            {storefrontSettings.socialTwitter && (
              <a href={storefrontSettings.socialTwitter} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-neutral-900 flex items-center justify-center text-neutral-400 hover:text-cyan-400 hover:bg-cyan-500/10 transition-colors">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
        ...
        <div className="lg:col-span-2">
          ...
          <ul className="space-y-4">
            {storefrontSettings.contactAddress && (
              <li className="flex items-start space-x-3">
                <MapPin className="w-5 h-5 text-cyan-400 flex-shrink-0 mt-0.5" />
                <span className="text-neutral-400 text-sm leading-relaxed whitespace-pre-line">
                  {storefrontSettings.contactAddress}
                </span>
              </li>
            )}
            {storefrontSettings.contactPhone && (
              <li className="flex items-center space-x-3">
                ...
                <span className="text-neutral-400 text-sm">{storefrontSettings.contactPhone}</span>
              </li>
            )}
            {storefrontSettings.contactEmail && (
              <li className="flex items-center space-x-3">
                <Mail className="w-5 h-5 text-cyan-400 flex-shrink-0" />
                <span className="text-neutral-400 text-sm">{storefrontSettings.contactEmail}</span>
              </li>
            )}
          </ul>
        </div>
